...
def main():
    st.set_page_config(page_title="AI Powered Web Application for Football Tactical Analysis", layout="wide",
    ...
    # Define the full absolute paths for your demo videos
    demo_vid_paths = {
        "Demo 1": r"E:\bishemai\Football-Analytics-with-Deep-Learning-and-Computer-Vision-master\Streamlit web app\demo_vid_1.mp4",
        "Demo 2": r"E:\bishemai\Football-Analytics-with-Deep-Learning-and-Computer-Vision-master\Streamlit web app\demo_vid_2.mp4"
    }

    # Select the demo video based on user choice
    demo_vid_path = demo_vid_paths[demo_selected]

    # Define the team info for each demo
    demo_team_info = {
        "Demo 1": {
            "team1_name": "France",
    ...
    # Select the team info based on the demo chosen
    selected_team_info = demo_team_info[demo_selected]

    # Create a temporary file and set its name to the selected demo video path
    tempf = tempfile.NamedTemporaryFile(suffix='.mp4', delete=False)
    if not input_vide_file:
        tempf.name = demo_vid_path  # Set the temporary file name to the demo video path
        demo_vid = open(tempf.name, 'rb')
        demo_bytes = demo_vid.read()

        # Display the demo video in the sidebar
    ...
    model_players = YOLO(
        r"E:\bishemai\Football-Analytics-with-Deep-Learning-and-Computer-Vision-master\models\Yolo8L Players\weights\best.pt")

    # Load the YOLOv8 field keypoints detection model
    model_keypoints = YOLO(r"E:\bishemai\Football-Analytics-with-Deep-Learning-and-Computer-Vision-master\models\Yolo8M Field Keypoints\weights\best.pt")
    ...
    with tab2:
        ...
        with t1col1:
            ...
            st.markdown('---')
            radio_options = [f"{team1_name} P color", f"{team1_name} GK color", f"{team2_name} P color",
                             f"{team2_name} GK color"]
            ...
            with cp1:
                hex_color_1 = st.session_state[
                    f"{team1_name} P color"] if f"{team1_name} P color" in st.session_state else selected_team_info[
                    "team1_p_color"]
                team1_p_color = st.color_picker(label=' ', value=hex_color_1, key='t1p')
                st.session_state[f"{team1_name} P color"] = team1_p_color
            with cp2:
                hex_color_2 = st.session_state[
                    f"{team1_name} GK color"] if f"{team1_name} GK color" in st.session_state else selected_team_info[
                    "team1_gk_color"]
                team1_gk_color = st.color_picker(label=' ', value=hex_color_2, key='t1gk')
                st.session_state[f"{team1_name} GK color"] = team1_gk_color
            with cp3:
                hex_color_3 = st.session_state[
                    f"{team2_name} P color"] if f"{team2_name} P color" in st.session_state else selected_team_info[
                    "team2_p_color"]
                team2_p_color = st.color_picker(label=' ', value=hex_color_3, key='t2p')
                st.session_state[f"{team2_name} P color"] = team2_p_color
            with cp4:
                hex_color_4 = st.session_state[
                    f"{team2_name} GK color"] if f"{team2_name} GK color" in st.session_state else selected_team_info[
                    "team2_gk_color"]
    ...
    colors_dic, color_list_lab = create_colors_info(team1_name, st.session_state[f"{team1_name} P color"],
                                                    st.session_state[f"{team1_name} GK color"],
                                                    team2_name, st.session_state[f"{team2_name} P color"],
                                                    st.session_state[f"{team2_name} GK color"])

    with tab3:
        t2col1, t2col2 = st.columns([1, 1])
        with t2col1:
            player_model_conf_thresh = st.slider('PLayers Detection Confidence Threshold', min_value=0.0, max_value=1.0,
                                                 value=0.6)
            keypoints_model_conf_thresh = st.slider('Field Keypoints PLayers Detection Confidence Threshold',
                                                    min_value=0.0, max_value=1.0, value=0.7)
            keypoints_displacement_mean_tol = st.slider('Keypoints Displacement RMSE Tolerance (pixels)', min_value=-1,
                                                        max_value=100, value=7,
        ...
        with t2col2:
            ...
            if save_output:
                output_file_name = st.text_input(label='File Name (Optional)',
        ...
        with bcol1:
            nbr_frames_no_ball_thresh = st.number_input("Ball track reset threshold (frames)", min_value=1,
                                                        max_value=10000,
                                                        value=30,
                                                        help="After how many frames with no ball detection, should the track be reset?")
            ball_track_dist_thresh = st.number_input("Ball track distance threshold (pixels)", min_value=1,
                                                     max_value=1280,
                                                     value=100,
                                                     help="Maximum allowed distance between two consecutive balls detection to keep the current track.")
            max_track_length = st.number_input("Maximum ball track length (Nbr. detections)", min_value=1,
                                               max_value=1000,
                                               value=35,
        ...
        with bcol2:
            ...
            with bcol21t:
                ...
                show_heatmap = st.toggle(label="Show Heatmap", value=True)
    ...
    if start_detection and not stop_detection:
        ...
        status = detect(cap, stframe, output_file_name, save_output, model_players, model_keypoints,
                        ...
                        num_pal_colors, colors_dic, color_list_lab, stats_placeholder=stats_placeholder)
    else:
        try:
